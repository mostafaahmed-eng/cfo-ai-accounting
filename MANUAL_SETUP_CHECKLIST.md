...
| `ENCRYPTION_KEY` | `cfo-backend/.env.example` line 34 | Generate locally | `python -c "from cryptography.fernet import Fernet; print(Fernet.generate_key().decode())"` — must be a valid 32-byte URL-safe base64 Fernet key ending in `=`. Now consumed by `app/core/crypto.py` for encryption at rest and enforced (production) in `app/config.py`. | **Required** — production boot fails without a valid key |
...
| `NEXT_PUBLIC_API_BASE_URL` | `cfo-dashboard/.env.local.example` line 1 (copy to `cfo-dashboard/.env.local`) | Leave as-is for local dev; **unset in production** | Value `http://localhost:8000/api/v1` is correct for local Docker Compose dev. Read by `cfo-dashboard/src/lib/api-client.ts` as the Axios base URL (same-origin `/api/v1` when unset/empty — the production default). Must include the `/api/v1` suffix when set. In production leave it unset so the browser uses the nginx origin. | Safe as-is for dev / unset for prod |
| **First user account** | `create_admin.py` | Run in the backend container | There is no user registration API endpoint (only login at `POST /api/v1/auth/login`). After `docker compose up`, create the first user with the bundled script, which requires an explicit password and validates it against the password policy (min 8 chars, upper + lower + digit) without ever printing it: `docker compose exec backend python create_admin.py --email admin@example.com --name "Admin" --password '<your-strong-password>'`. | **Required** — login endpoint returns 401 for every request without a seeded user |
...
| `TELEGRAM_BOT_TOKEN` | `cfo-backend/.env.example` line 21 | Telegram — @BotFather | 1. Open Telegram and search for `@BotFather`. 2. Send `/newbot`. 3. Follow the prompts to choose a name and username. 4. BotFather will reply with a token — copy it without sharing or logging it. 5. Set it as `TELEGRAM_BOT_TOKEN`. The Telegram client uses it to send bot responses. | **Optional** — Telegram integration is not needed for core features |
| `TELEGRAM_WEBHOOK_SECRET` | `cfo-backend/.env.example` | Generate locally | `python -c "import secrets; print(secrets.token_urlsafe(32))"` — this is the shared secret Telegram includes in the `X-Telegram-Bot-Api-Secret-Token` header. It must match the secret used when registering the webhook. | **Required for Telegram outside an explicitly enabled local-development bypass** |
...
| `PLATFORM_ADMIN_EMAILS` | `cfo-backend/.env.example` line 13 | Comma-separated emails of users you trust as platform operators | Only these users may change the **global** Telegram bot configuration (`PUT /api/v1/integrations/telegram/bot-config`, the Settings → Telegram page). Company-level roles never grant this. Leave empty to deny everyone. | **Required** if you let users change the bot config from the dashboard |
| `TELEGRAM_PAIRING_TTL_MINUTES` | `cfo-backend/.env.example` | Choose an expiry window | Default `15`. Pairing links expire after this many minutes and are single-use. | Safe as-is |
| `TELEGRAM_ALLOW_INSECURE_LOCAL_WEBHOOK` | `cfo-backend/.env.example` | Leave `false` except for isolated local development | Setting this to `true` bypasses the webhook secret only when `ENVIRONMENT=development`. Never enable it in staging or production. | Safe default is `false` |
...
| Variable / Item | File & path | Where to get it | Exact steps to obtain it | Required for local dev? |
|---|---|---|---|---|
| **Register Telegram webhook URL** | Endpoint: `POST /api/v1/telegram/webhook` | Telegram Bot API | After the backend is running and `TELEGRAM_BOT_TOKEN` and `TELEGRAM_WEBHOOK_SECRET` are set, register the webhook by calling: `curl -X POST "https://api.telegram.org/bot<YOUR_BOT_TOKEN>/setWebhook" -H "Content-Type: application/json" -d '{"url": "https://<YOUR_PUBLIC_URL>/api/v1/telegram/webhook", "secret_token": "<YOUR_TELEGRAM_WEBHOOK_SECRET>"}'`. For local development you need a public HTTPS tunnel. The application does not register or replace the real webhook automatically. | **Optional unless using Telegram** |
| **Pair a company chat** | Telegram Settings in the dashboard | Application + Telegram | Click **Connect Bot**, then use the one-time link or send `/start <single-use-code>` to the configured bot before the displayed expiry. Requesting a new code revokes the prior unused code. A message without a valid code cannot claim a pending integration. | **Required for each Telegram company connection** |
...
| `TELEGRAM_BOT_TOKEN` | 2 | Obtain from @BotFather and keep it secret |
...
- **`TELEGRAM_BOT_TOKEN`** is read by the Telegram client for outbound bot responses. Keep it
  only in ignored local environment files or the production secret store.
