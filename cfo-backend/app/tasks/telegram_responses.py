...
def send_telegram_response(self, chat_id: int, text: str, reply_markup: dict | None = None):
    try:
...
@celery_app.task(bind=True, max_retries=3, default_retry_delay=10)
def send_telegram_edit(self, chat_id: int, message_id: int, text: str, reply_markup: dict | None = None):
    try:
        result = _run_async(telegram_client.edit_message_text(chat_id, message_id, text, reply_markup))
        if not result.get("ok"):
...
@celery_app.task(bind=True, max_retries=3, default_retry_delay=10)
def answer_telegram_callback(self, callback_query_id: str, text: str | None = None, show_alert: bool = False):
    try:
        result = _run_async(telegram_client.answer_callback_query(callback_query_id, text, show_alert))
        if not result.get("ok"):
