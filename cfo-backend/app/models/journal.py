from sqlalchemy import Column, String, ForeignKey, Numeric, DateTime, Date, CheckConstraint, UniqueConstraint
...
class JournalEntry(BaseModel, TimestampMixin):
    ...
    reversed_entry_id = Column(UUID(as_uuid=True), ForeignKey("journal_entries.id"), nullable=True)

    company = relationship("Company", back_populates="journal_entries")
    lines = relationship("JournalLine", back_populates="journal_entry", cascade="all, delete-orphan", lazy="selectin")
    poster = relationship("User", foreign_keys=[posted_by], lazy="noload")
    reversed_entry = relationship("JournalEntry", remote_side="JournalEntry.id", foreign_keys=[reversed_entry_id], lazy="noload")
